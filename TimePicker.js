export function activate(){
  ...
  document.querySelectorAll(".time-pickable").forEach(timePickable => {
    ...
    timePickable.addEventListener("focus", buildAndShowPicker);
    timePickable.addEventListener("input", buildAndShowPicker);
  });
}

export function show(timePickable){
...
export function buildPicker(timePickable){
...
export function getTimePartsFromPickable(timePickable){
...
export function getSelectsFromPicker(timePicker){
...
export function getTimeStringFromPicker(timePicker){
  const selects = getSelectsFromPicker(timePicker)
  return `${selects.hour.value}:${selects.minute.value} ${selects.meridiem.value}`;
}

export function numberToOption(number){
  const padded = number.toString().padStart(2,"0");
  
  return `<option value="${padded}">${padded}</option>`;
} 

// const egTimePickable = document.querySelector(".time-pickable"); 